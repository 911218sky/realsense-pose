...
class SurgeryInfo(BaseModel):
    ...
    surgery_date: Optional[date] = Field(None, description="開刀日期（若未知可為 None）。")
...
class RehabTreatmentInfo(BaseModel):
    ...
    self_pay_items: Optional[str] = Field(None, description="自費項目（文字；若無可不填）。")
    other_items: Optional[str] = Field(None, description="其它治療/項目（文字；若無可不填）。")
...
class LifestyleInfo(BaseModel):
    ...
    vigorous_10min: Optional[bool] = Field(None, description="是否曾有『費力』運動 >=10 分鐘。")
    vigorous_60min_per_week: Optional[bool] = Field(None, description="每週費力運動累積是否 >=60 分鐘。")
    moderate_10min: Optional[bool] = Field(None, description="是否曾有『中等費力』運動 >=10 分鐘。")
